...
(function(ns){
  ...
  function assistMode(scene){var mode=(scene&&scene._assistMode)||(ns.RunState&&ns.RunState.assistMode),save;if(!mode&&ns.SAVE&&ns.SAVE.boot&&ns.SAVE.assistMode){save=ns.SAVE.boot();mode=ns.SAVE.assistMode(save)}return mode||{}}

  function polish(scene,node,duration,drift){var mode=assistMode(scene),alpha,angle;if(!node||!scene||!scene.tweens||!scene.tweens.add)return node;if(!mode.reduceFlash){alpha=node.alpha==null?1:node.alpha;if(node.setAlpha)node.setAlpha(0);else node.alpha=0;scene.tweens.add({targets:node,alpha:alpha,duration:duration})}if(drift){angle=mode.reduceShake?0.6:2;scene.tweens.add({targets:node,angle:angle,duration:3000,yoyo:true,repeat:-1,ease:'Sine.easeInOut'})}return node}
  ...
  function showTitle(scene){
    ...
    if (!bg && scene.add && scene.add.rectangle) {
      bg = setScroll(scene.add.rectangle(width / 2, height / 2, width, height, 0x0b0d12, 1).setDepth(0));
    }
    prompt = addText(scene, width / 2, height * 0.72, 'PRESS ANY KEY.', 12, 4);
    polish(scene, bg, 400, false);
    polish(scene, logo, 400, true);
    polish(scene, prompt, 400, false);
  }
  ...
  function showGameOver(scene){
    ...
    for (var i = 0; i < nodes.length; i++) polish(scene, nodes[i], 250, false);
